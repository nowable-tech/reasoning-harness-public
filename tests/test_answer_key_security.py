...
(currently named `answer_key`) must never survive into the object sent to a
model.
...
Run: python3 -m unittest tests.test_answer_key_security -v
"""
...
class TestAnswerKeyNeverReachesRequestPath(unittest.TestCase):
    def test_load_prompts_strips_answer_key_from_every_entry(self):
        prompts = load_prompts()
        self.assertGreater(len(prompts), 0, "sanity check: prompts.yaml loaded something")
        for pid, entry in prompts.items():
            self.assertNotIn("answer_key", entry, f"{pid}: answer_key leaked into the request-path object")

    def test_load_multilang_prompts_strips_answer_key_from_every_entry(self):
        prompts = load_multilang_prompts()
        self.assertGreater(len(prompts), 0, "sanity check: prompts_multilang.yaml loaded something")
        for pid, entry in prompts.items():
            self.assertNotIn("answer_key", entry, f"{pid}: answer_key leaked into the request-path object")

    def test_load_prompts_send_field_is_present_and_answer_key_free(self):
        # Belt-and-braces: even nested/serialized, the string "answer_key" should
        # not appear anywhere in a prompt's send-path representation.
        import json
        prompts = load_prompts()
        for pid, entry in prompts.items():
            serialized = json.dumps(entry)
            self.assertNotIn('"answer_key"', serialized, f"{pid}: answer_key key present in serialized request object")
