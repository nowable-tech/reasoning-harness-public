"""Thinking Machines Inkling adapter — trace_exposure: raw (expected).

Runs thinkingmachines/inkling through the OpenRouter OpenAI-compatible
gateway. No direct provider API exists for this model — Thinking Machines
names five third-party hosting partners (Together, Fireworks, Modal,
Databricks, Baseten); OpenRouter may route across more than one of them.
Preflight (2026-07-19) saw served_by="Together" on every one of 9 probe
calls, but that is not a guarantee for future calls the way it is for a
single-host model — log served_by on every row and flag any deviation.

include_reasoning=True + reasoning.effort=<level> in extra_body, same
convention as mistral_adapter.py. Reasoning arrives either as
msg.reasoning / msg.reasoning_content (OpenRouter native field, confirmed
present in preflight) or inside <think>...</think> tags — both checked.
"""
...
class ThinkingMachinesAdapter(BaseAdapter):
    required_env = ["OPENROUTER_API_KEY"]

    def _check_credentials(self) -> None:
        if not os.environ.get("OPENROUTER_API_KEY"):
            raise CredentialMissingError(
                f"{self.model_key}: missing OPENROUTER_API_KEY (Inkling runs via OpenRouter, no direct API)"
            )
    ...
    def call(self, prompt: str, thinking_budget: int = 4096, reasoning_effort: str = "high") -> ModelResponse:
        ...
        try:
            ...
            raise AdapterError(f"{self.model_key} API error: {exc}") from exc
        ...
        if reasoning:
            ...
            trace_status = "count_only"  # OpenRouter did not forward text — MISMATCH vs expected raw
        else:
            trace_status = "absent"
